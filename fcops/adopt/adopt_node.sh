...
CONFIG_FILE=/opt/flight/deployment/fcops/adopt/adopt_config
...
#Keys from cgw1 -> nodes should exist on cloud vik so:
cat /opt/flight/deployment/fcops/adopt/fcops_setup.sh |bash -s "$NEW_NODE"
#ssh root@chead1 "curl http://cfcgateway/resources/adoption/fcops_setup.sh |bash -s "$NEW_NODE""

# Then Zabbix install / setup / config

bash /opt/flight/deployment/fcops/adopt/zabbix_setup.sh "$NEW_NODE"
#bash /opt/zabbix/srv/resources/adoption/zabbix_setup.sh "$NEW_NODE"

#Tell Slack we've picked up a new node / been adopted

bash /opt/flight/deployment/fcops/adopt/slack_msg.sh "$NEW_NODE"
